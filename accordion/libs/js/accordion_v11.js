...
/*
	1. 먼저 jQuery 플러그인 문법에 맞게 플러그인 몸체인 folderAccordionMenu() 프러그인을 만듦.
	1-1. 내부에는 선택자 개수만큼 FolderAccordionMenu 클래스 인스턴스를 생성.
	1-2. $('#accordionMenu1').folderAccordionMenu().on(...)과 같이 folderAccordionMenu() 플러그인 호출 후 on()과 같은 jQuery 메서드를 호출할 수 있게 this를 리턴.
	2. 기존 폴더아코디언 인스턴스 생성 코드와 이벤트 리스너 추가 코드를 주석 처리 또는 삭제 후 folderAccordionMenu 플러그인을 활용해 jQuery 방식으로 코드 변경.
*/
